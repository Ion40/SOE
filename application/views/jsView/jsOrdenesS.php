...
$(document).ready( function () {
	...
	var calculo = 0;
	var table = $("#tblNuevaOrdenS").DataTable();
...
$("#tblOrdenesS").on("click","tbody .detalles", function(){
	var table = $("#tblOrdenesS").DataTable();
		...
		success:function(response){
			...
			if (response != "false") {
				...
				$.each(JSON.parse(response), function(i, item){
					tbody += "<tr>"+
						"<td><a href='DetallesOrderS/"+item["Ruta"]+"/"+item["FechaEntrega"]+"'>"+item["Ruta"]+"</a></td>"+
...
function DetallesModal(fecha,idcliente,vendedor,nombrecliente)
{
	...
	$("#tblDetalleOrderS").DataTable({
		"ajax": ruta+"../DetallesOrderSModal/"+fecha+"/"+idcliente,
		responsive:true,
		"autoWidth":false,
		"info": true,
		"sort":true,
...
function DetOrdSxCli(cliente,fecha,vendedor,nombrecliente,estado)
{
	var loc = window.location;
	var pathName = loc.pathname.substring(0, loc.pathname.lastIndexOf('/') + 3);
	var ruta = loc.href.substring(0, loc.href.length - ((loc.pathname + loc.search + loc.hash).length - pathName.length));
	...
	$("#tblEditarOrdenS").DataTable({
		"ajax": ruta+"../DetOrderSxCli/"+cliente+"/"+fecha,
		responsive:true,
		"autoWidth":false,
		"info": false,
		"sort":true,
...
function cambiarUnidades(){
	var table = $("#tblEditarOrdenS").DataTable();
	var calculo;
	table.rows().eq(0).each(function (index) {
		var row = table.row(index);
		var data = row.data();
...
function remcli (cliente,ruta,fecha) {
	...
	this.location.href = ruta1+"../remisionOrdenSCliente/"+cliente+"/"+ruta+"/"+fecha;
}
